...
/*
 * PartnerScripts — centralized, ENV-GATED third-party tags. DISABLED by default (local dev + build) so NO
 * external script fires locally and there are NO live GAM/AdSense/iZooto/GA/GTM calls.
 *
 ...
 * canary is. The flag is gone; each system now has its own, and each is fail-closed.
 *
 * GPT is loaded by `components/ads/GamBootstrap.tsx`, and only after the tester opens the session gate. That
 * is a THREE-condition load (GAM enabled + canary mode + tester action); AdSense below is a one-condition
 * load and must stay `false` for the canary, because a second ad system on the same page would make every
 ...
 *
 * This component renders nothing unless a flag is explicitly "true", so it is inert in dev.
 */
